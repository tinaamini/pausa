...
import 'package:pausa/features/apps/widgets/dialog_widget.dart';
...
class _AppsViewState extends State<_AppsView>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      context.read<AppsCubit>().onAppResumed();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AppsCubit, AppsState>(
      ...
      curr.permissionDeniedCount > prev.permissionDeniedCount,
      listener: (context, state) {
        final cubit = context.read<AppsCubit>();
        showDialog(
          context: context,
          barrierDismissible: false,
          builder: (_) => DialogWidget(cubit:cubit),
        );
      },
